...
static int hotNewsCurrentPageIndex = 0; // 当前分页请求的页面号
static const int hotNewsPageSize = 10; // 每次分页请求的页面大小
// 获取request url
static NSString* hotNewsNetworkRequestURL(int pageIndex, int pageSize)
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    dateFormatter.dateFormat = @"yyyy-MM-dd'T'HH:mm:ss.SSS";
    NSDate *endDate = [NSDate date];
    NSDate *startDate = [NSDate dateWithTimeIntervalSinceNow:-7*24*60*60]; // 一个星期,通过大数据分析，这个时间长度最合适:)
    NSString *endDateString = [dateFormatter stringFromDate:endDate];
    NSString *startDateString = [dateFormatter stringFromDate:startDate];
    
    return [NSString stringWithFormat:@"http://api.cnblogs.com/api/newsitems/@hot?startDate=%@&endDate=%@&pageIndex=%d&pageSize=%d", startDateString, endDateString, pageIndex, pageSize];
}

@interface ICNewsHotTableView () <UITableViewDelegate>

@property (nonatomic, strong) ICTableViewDataSource *hotNewsDataSource;
@property (nonatomic, strong) NSMutableArray *items;

@end

@implementation ICNewsHotTableView
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.delegate = self;
        self.dataSource = self.hotNewsDataSource;
...
- (void)setupTableViewRefreshEvents
{
    ...
    self.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        hotNewsCurrentPageIndex = 1;
        NSString *requestURL = hotNewsNetworkRequestURL(hotNewsCurrentPageIndex, hotNewsPageSize);
        [ICNetworkTool loadDataWithURL:requestURL success:^(id responseObject) {
            ...
            // 更新tableView视图
            weakSelf.hotNewsDataSource.items = weakSelf.items;
            [weakSelf reloadData];
            // 结束下拉刷新
            [weakSelf.mj_header endRefreshing];
        } failure:^(NSError *error) {
            ICLog(@"hot news load new data failure:%@", error);
            [weakSelf.mj_header endRefreshing];
        }];
    }];
    ...
    self.mj_footer = [MJRefreshAutoFooter footerWithRefreshingBlock:^{
        ...
        [ICNetworkTool loadDataWithURL:requestURL success:^(id responseObject) {
            ...
            if ([(NSArray *)responseObject count] < hotNewsPageSize) {
                hotNewsCurrentPageIndex--;
            ...
            ICLog(@"hot news load more data failure:%@", error);
            [weakSelf.mj_footer endRefreshing];
        }];
    }];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    newsContentViewController.pushedViewControllerId = 2;
...
- (ICTableViewDataSource *)hotNewsDataSource
{
    if (_hotNewsDataSource == nil) {
        _hotNewsDataSource = [[ICTableViewDataSource alloc] initWithItems:self.items cellIdentifier:@"ICNewsTableViewCellIdentifier" cellType:@"ICNewsTableViewCell" configureBlock:^(ICNewsTableViewCell *cell, ICNews *item) {
